...
OG.geometry.PolyLine.prototype.isRightAngleBetweenPoints = function (prev, next) {
    var horizontalAngles = [0, 180, 360];
    var verticalAngles = [90, 270];
    var angleDeg = this.angleBetweenPoints(prev, next);
...
OG.geometry.PolyLine.prototype.angleBetweenThreePoints = function (prev, center, next) {
    ...
    var BC = Math.sqrt(Math.pow(center.x-next.x,2)+ Math.pow(center.y-next.y,2));

    var AC = Math.sqrt(Math.pow(next.x-prev.x,2)+ Math.pow(next.y-prev.y,2));
